...
# dir_custom = '~/github/ohi-webapps/custom/shp'
...
shp = rgdal::readOGR(dsn = file.path(dir_spatial, 'custom/raw'),
...
writeOGR(shp, dsn = file.path(dir_spatial, 'custom'), 
         layer = 'chl_shp', driver = 'ESRI Shapefile', overwrite=T) 

## save rgn_offshore_data.csv on git-annex
write.csv(shp@data, file.path(dir_spatial, 'rgn_offshore_data.csv'), row.names=FALSE)

# save lookup table with unique rgn_ids on github
write.csv(shp@data, sprintf('~/github/ohi-webapps/custom/%s/rgns_custom.csv', key), row.names=FALSE)
...
```{r prep shp map custom_maps, eval=FALSE}
...
## 4. shp layers: bind possible layers for shp


```{r shp layers: bind from shp-xxx repos, eval=F}

# clone existing shp repo
key = 'shp'
setwd(dir_repos)
...
lkp = read.csv('~/github/ohi-webapps/custom/shp/col_rgns_custom.csv')
...
for (f in tounlink) {
  unlink(file.path(dir_repos, 'shp/subcountry2014/layers', f), 
         recursive=T, force=T)
}

# list all layers to copy to shp repo
lyrs_list = list.files(file.path(dir_repos, 'shp/subcountry2014/layers'), # was orignally subcountry2014
                       glob2rx('*.csv'), full.names=T) 


# loop through all layers, all shp repos to create correct # of regions per shp layer
...
## 5. shp layers: handle problem layers 

Handle several problem layers individually 

```{r col_layers: handle individuals, eval=FALSE}

dir_col_lyrs = file.path(dir_repos, 'shp/subcountry2014/layers')
l_csv = file.path(dir_repos, 'shp/subcountry2014/layers.csv')
...
for (f in c(f_js, f_geojson)){ # f = f_spatial[1]
  if (key != 'shp')  file.copy(f, sprintf('spatial/%s', basename(f)), overwrite=T) # original
  if (key == 'shp') file.copy(f, sprintf('subcountry2014/spatial/%s', basename(f)), overwrite=T) # shp
...
(Note: no need to also overwrite `.travis.yml` since based on existing `shp` repo)
...
f_in  = file.path(dir_repos, 'shp', 'subcountry2014/conf/config.r')
f_out = f_in 
...
## 8. Test shp's calculate_scores.r

Make sure no errors in calculating goal scores. 

```{r test shp calculate_scores, eval=F}

setwd(file.path(dir_repos, 'shp', 'subcountry2014'))
