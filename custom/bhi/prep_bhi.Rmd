...
dir_tmp_baltic = '~/github/ohi-webapps/tmp_baltic/_bhi_current'
...
# cp command: `cp -r from to`. the -r means 'including subdirectories'. The `/.` means contents only, not folder itself
 system(sprintf("cp -r %s %s",
                file.path(dir_repos, 'bhi'), dir_tmp_baltic)) 
```
...
# cp command: `cp -r from to`. the -r means 'including subdirectories'. The `/.` means contents only, not folder itself
 system(sprintf("cp -r %s/. %s", 
                dir_tmp_baltic, file.path(dir_repos, 'bhi'))) 
...
#  f_out = sprintf('conf/%s', f)
 ...
 f_out = f_in #file.path(dir_repo,  'bhi', 'baltic2015/conf/config.r'
