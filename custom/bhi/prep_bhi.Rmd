...
require(maptools) # readShapePoly preferred to readOGR: gis.stackexchange.com/questions/113964/fixing-orphaned-holes-in-r
...
# gives this error: 
#   Error in pmax(nchar(ln), nchar(minv), nchar(maxv)) : 
...
# bhiplc = readShapePoly(file.path(dir_neptune, 'git-annex/clip-n-ship/bhi/spatial/custom/raw',
                              # 'Inters_BALTIC_EEZ_PLC1'))
...
if (redo_shp) {
  ...
  bhi@data = bhi@data %>%
    ...
    mutate(rgn_name = paste(substr(cntry_name, 1, 3),
                            '-',
                            basin_name), sep=' ') %>%
    select(rgn_id, area_km2, cntry_name, basin_name, rgn_name) # since 'sep' column exists
  
  # check for errors in shp file: 'orphaned hole' error encountered in Step 9 below 
  if (clean_spatial) source('~/github/ohi-webapps/custom/bhi/clean_spatial_bhi.r')
...
## 9. proceed create_all.r - custom_maps() : 
