...
# from create.all
source(sprintf('%s/ohi-webapps/ohi-travis-functions.r', dir_github))
setwd(dir_repo)
push_branch('draft') 
system('git pull')
...
And from terminal, run the following. 
This is based on what the cron job does, see https://github.com/OHI-Science/issues/issues/512. 
...
/srv/shiny-server$ wget http://ohi-science.nceas.ucsb.edu/bhi > /dev/null 2>&1
  
```


# archive from prep_bhi.r
