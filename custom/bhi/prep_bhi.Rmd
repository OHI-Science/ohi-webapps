...
This script recreates the Baltic Health Index (BHI) repository with 42 regions instead of 25. See Issue #407: https://github.com/OHI-Science/issues/issues/407. Note: This script superceded `prep_bhi.r`, which originally created the BHI WebApp in April 2015 with PLC Basin *Inters_BALTIC_EEZ_PLC1* regions. `prep_bhi.rmd` here recreates the BHI WebApp in August 2015 with HOLAS Basin *Intersect_HELCOMsubbasins_BALTIC_EEZ_excl_small_poly* regions. 

# Overview
Not all steps were necessary to run this time, since the `bhi` repo already existed and was just being edited. Each R chunk below will not be evaluated (ie, `{r, eval=F}). Steps below that were run when recreating `bhi`: 

- 1. setup
- 2. prepping map from received shape files  
- 3. prepping map with custom_maps()

## 0. add all Baltic countries to custom/bhi/sc_studies_custom_bhi.csv by hand
This has the same structure as `custom/sc_studies_custom.csv`. 
...
```{r prep map from shapefile, eval=F}
...
bhi_tmp = readOGR(dsn = file.path(dir_annex, 'bhi/spatial/custom/raw'),
...
bhi = readShapePoly(file.path(dir_annex, 'bhi/spatial/custom/raw',
...
writeOGR(bhi, dsn = file.path(dir_annex, 'bhi/spatial/custom'), 
...
```{r prep map custom_maps, eval=FALSE}
...
## 6. run ohi-webapps/create_all.r through populate_draft_branch()
