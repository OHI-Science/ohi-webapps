...
banana   = readOGR(dsn='spatial', layer = 'banana')%>%spTransform(laeaCRS)
...
banana@data = banana@data%>%
                mutate(rgn_name = 'banana',
                       country = ' NA')%>%
                select(rgn_name,country)
...
all = rbind.SpatialPolygonsDataFrame(alaska, canada, russia, svalbard, norway, janmayen, greenland,banana)
...
writeOGR(all,dsn='spatial',layer='arctic_eezs',driver="ESRI Shapefile",overwrite_layer = TRUE)
