...
Each R chunk below will not be evaluated (ie, `{r, eval=F}`). Steps below that were run when recreating `bhi`: 
...
dir_annex = file.path(dir_neptune, 'git-annex')
...
## 2. prep BC map from @oharac's shape files  
...
bc = readOGR(dsn = file.path(dir_annex, 'bcprep/regions'),
              layer = 'ohibc_rgn')
plot(bc)

# COME BACK HERE and name scenario in sc_studies_custom.csv
# @oharac will add km2 to .dbf file
...
writeOGR(bc, dsn = file.path(dir_annex, 'bc/spatial/custom'), 
