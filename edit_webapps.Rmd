...
- create intermediate repos for each as a normal WebApp (in hindsight this was unnecessary; could have just used existing repos but there was a reason I can't remember why I opted not to...)
- create repo for bhi as a whole
- populate bhi repo with information for regions-within-baltic-countries

```{r redo bhi repo+webapp, eval=F}

# see ohi-webapps/custom/bhi/prep_bhi.r
...
- create arc repo
- copy the ohi-global repo into arc
- filter all layers for just the proper Arctic countries
...
- create repo named arc (`create_functions.r - rename_gh_repo` exists too if I need it)
- make maps
