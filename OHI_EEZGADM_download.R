## OHI_EEZGADM_download.r
## download OHI GADM boundaries and save
## based on code below by Charlotte Marcinko; archived at bottom. 
...
log_not_downloaded = data_frame()

## for each sc_key
for (key in sc_studies$sc_key) { # key = "bra"
  
  ## trycatch to skip to next sc_key if data is missing from archive
  possibleError<-tryCatch({
    
    ## set destination file to save downloaded file
    file_save <- path.expand(paste0('~/github/ohi-webapps/ohi_gadm_geojson_archive/regions_gcs_', key , '.geojson'))
    
    ## set url for to download file
    file_url <- paste("https://raw.githubusercontent.com/OHI-Science",
                      key, "draft/subcountry2014/spatial/regions_gcs.geojson", sep="/")
    
    ## download geojson file for key
    download.file(file_url, file_save, method = "curl")
    
  },error=function(e) e) # end trycatch
  
  
  ## if error in download skip to next but record
  if(inherits(possibleError, "error")) {
   
    log_not_downloaded = log_not_downloaded %>%
      bind_rows(data_frame(key))
