...
## moved to R/calculate_scores_check()
...
update_pages <- function(){
  ...
  if (dir.exists(dir_webapps_templates)) {
    ...
    for (f in c('regions.brew.md','layers.brew.md','goals.brew.md','scores.brew.md')){
      url_in =  file.path('https://raw.githubusercontent.com/OHI-Science', key, 'draft', dir_webapps_templates, f) 
      f_out  = file.path(dir_brew, f)
      writeBin(httr::content(GET(url_in)), f_out)
    }
