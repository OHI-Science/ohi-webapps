...
wd  = '~/github/<%=git_repo%>/<%=default_scenario%>'
...
# <%=name_gl_rgn%>'s region_id in the global assessment is <%=gl_rgn_id%>
scores_global_<%=name_gl_rgn%> = scores_v2014.3 %>%            
  filter(region_id == <%=gl_rgn_id%>) %>%
  mutate(goal = as.character(goal))


## read in <%=git_repo%>/<%=default_scenario%> scores         
...
scores_compare = scores_global_<%=name_gl_rgn%> %>%   
  select(goal, 
         dimension,
         score_global = score) %>%         
