...
1. Set the repo key (an example for the Baltic: `key <- 'bhi'`)
2. Set the directory where you want to work locally on your computer
...
## build working directory for repo 
dir_repo  <- file.path(dir_temp, key)

## read in repo registry information
...
ohirepos::deploy_website(key, dir_repo, clone = TRUE, push = TRUE) 
```
...
repo <- ohirepos::populate_layers(key, dir_repo, repo_registry, multi_nation = FALSE)
...
# TODO: remove ohicore@dev dependency
```
...
commit_and_push(key, 
                dir_repo, 
                commit_message = 'populating test2')
