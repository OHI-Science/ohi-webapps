...
# updates 2015-04-23 by bbest, jules32 ----
keys = 'abw' #sc_studies %>% filter(!is.na(sc_annex_dir)) %>% select(sc_key)
keys = keys[,1]
# update .travis.yml 
sapply(keys, update_travis_yml, 'update About using ohi-webapps/create_functions.R - update_travis_yml()')
